...
(function() {
    ...
    function ogController($window, $document) {
        ...
        function screenShot(){
            var cover_node = document.querySelector('.book_cover');

	    domtoimage.toPng(cover_node).then(function(dataUrl) {
		// ref: https://stackoverflow.com/a/50300880
		var link = document.createElement('a');
		var book_name = $('.book_title_below').text() + ' ' + $('.book_title_above').text() + ' (' + $('.book_version').text() + ')'
		link.download = book_name + '.png';
		link.href = dataUrl;
		link.click();
	    })
        }
